...
::: warning 구현 상태
서브셋 빌더와 벤치마크 하네스는 **아직 이 저장소에 없다**. 이 페이지는 그것들이 구현해야 할 설계를
명세한다. `recorded at run time`이라고 적힌 항목은 의도적으로 비워 둔 것이다. 손으로 입력하는 값이
아니라 실행이 만들어 내는 산출물에서 채워진다.
:::
...
- **반경 방향 편향** — 웨이퍼 edge 쪽으로 갈수록 불량을 체계적으로 과소 예측한다. 중심부 측정으로
  캘리브레이션된 모델의 전형적인 특징이다.
- **공간 평활화** — 미세 구조를 뭉갠다. 그래서 실제에 존재하는 국소 패턴이 prior에는 없다.
- **전역 오프셋과 이득** — 웨이퍼 전체에 적용된 캘리브레이션 오차.

편향 파라미터는 실행마다 고정되고, 결과 파일에 기록되며, 모든 전략 arm에서 동일하다.
