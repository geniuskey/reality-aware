...
WM-811K의 라벨은 이진이고, 실제 계측값은 그렇지 않다. 그래서 대체 웨이퍼 출처는 **연속값** 타깃도
생성할 수 있다. 매끄러운 표면(반경형, 기울기, 안장형, 링, 스팟, 스트라이프)에 다이별 잡음을 더한
것으로, CD·두께·오버레이 맵이 갖는 형태다.

```bash
python -m nano.benchmark --synthetic --target continuous --seeds 0 1 2 3 4
```

파이프라인의 나머지는 그대로다. 같은 prior 생성기, 같은 추정기, 같은 선택 규칙, 같은 예산이다.
달라지는 것은 지표가 말할 수 있는 범위다. 클래스 균형 지표는 균형 잡을 클래스가 없으므로, 연속값
실행은 단순 MAE를 앞세우고 RMSE를 함께 보고하며, 상수 참조는 "불량 없음" 대신 prior의 평균
수준을 예측한다.

계측이 실제로 만들어 내는 종류의 데이터에서 같은 선택 규칙이 통하는지 물을 수 있는 출처는 여기서
이것뿐이다. 답은 팹이 아니라 대체물이 내놓는 것이며, 그 실행이 무엇을 발견했는지는 — 이진 실행과
모순되는 결과 하나를 포함해 — [정직한 범위](./limitations)에 기록되어 있다.
