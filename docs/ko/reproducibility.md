...
```bash
npm run build:playground   # python -m nano.playground: 웨이퍼를 심어 페이지를 작성
npm run check:playground   # Python의 결정을 그 페이지 자신의 엔진으로 재생
```

첫 번째 명령은 스탠드인 웨이퍼로는 실행을 거부한다. 그 페이지는 독자에게 WM-811K를 보여주고 있다고
말하기 때문이다. 두 번째 명령이 페이지에 적히는 *트레이스 N/M 일치* 수치를 만들어 내며, 배포 때마다
CI에서 실행된다. 그래서 브라우저 이식본이 만들어 낸 불일치는 조용히 배포되지 않고 게시 자체를
실패시킨다.
...
WM-811K 배포본은 여기에 커밋되어 있지 않다 — 배포 링크와 이용 조건은 [데이터셋](./data)을
참고하세요. 예상되는 로컬 디렉터리 구조는 다음과 같다:
